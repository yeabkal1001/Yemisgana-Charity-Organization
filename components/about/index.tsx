...
export function About() {
  ...
  return (
    <section id="about" ref={containerRef} className="w-full bg-[var(--color-bg-secondary)] relative overflow-hidden select-none border-b border-[var(--color-border)] grid-bg">
      ...
      <div className="px-6 md:px-20 lg:px-24 pt-24 md:pt-32 pb-10 md:pb-14 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-6">
          ...
          <motion.p
            ...
            viewport={{ once: true }}
            className="text-[var(--color-text-muted-40)] font-sans text-xs md:text-sm max-w-xs leading-relaxed"
          >
      ...
      {/* ── Marquee ticker ───────────────────────────────────────── */}
      <div className="border-y border-[var(--color-border)] overflow-hidden py-3.5 bg-[rgba(var(--color-bg-primary),0.8)] backdrop-blur-sm relative z-10">
        <div className="flex whitespace-nowrap animate-marquee">
          {[...tickerItems, ...tickerItems].map((item, i) => (
            <span
              key={i}
              className="text-[var(--color-text-muted-60)] font-sans font-bold text-[10px] tracking-[0.25em] uppercase px-8"
            >
              {item}
      ...
      <div className="flex flex-col lg:flex-row relative z-10">
        ...
        >
          <div>
            <p className="text-[var(--color-text-muted-70)] font-sans text-base md:text-lg leading-[1.8] max-w-xl">
            ...
            <div className="mt-12 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-6 relative overflow-hidden group">
              <div className="absolute right-4 top-4 text-[var(--color-text-muted-5)] group-hover:text-lime/10 transition-colors duration-500">
                <Quote size={60} strokeWidth={1} />
              </div>
              <p className="text-[var(--color-text-muted-80)] font-serif italic text-[15px] md:text-[17px] leading-[1.6] relative z-10">
          ...
          <div className="mt-12">
            <p className="text-[var(--color-text-muted-30)] text-[9px] font-bold tracking-[0.25em] uppercase mb-4">Our Core Values</p>
              ...
              {values.map((v, i) => (
                <div
                  ...
                  className={`px-4 py-2 rounded-full border text-[10px] font-bold tracking-[0.18em] uppercase transition-all duration-300 cursor-default relative overflow-hidden ${
                    hoveredValue === i
                      ? 'border-lime text-[var(--color-bg-primary)] bg-lime scale-[1.03]'
                      : 'border-[var(--color-border)] text-[var(--color-text-muted-75)] bg-[var(--color-surface)] hover:border-[var(--color-text-muted-35)]'
                  }`}
                >
                  {v.name}
                </div>
            ...
            <div className="h-10 mt-4 relative overflow-hidden">
              {values.map((v, i) => (
                <motion.p
                  ...
                  className="absolute inset-0 text-[var(--color-text-muted-50)] text-[11px] font-sans leading-relaxed pointer-events-none"
                >
                  {v.desc}
                </motion.p>
              ))}
              {hoveredValue === null && (
                <p className="text-[var(--color-text-muted-20)] text-[11px] font-sans leading-relaxed italic">
        ...
        >
          <div className="relative w-full h-full min-h-[300px] md:min-h-[400px] rounded-3xl overflow-hidden group">
            ...
            <motion.div
              ...
              viewport={{ once: true }}
              className="absolute bottom-6 left-6 right-6 md:right-auto md:max-w-[270px] bg-[rgba(var(--color-bg-primary),0.85)] backdrop-blur-xl rounded-2xl p-5 border border-[var(--color-border)] shadow-2xl"
            >
              <p className="text-lime font-sans text-[9px] font-black tracking-[0.25em] uppercase mb-1.5">
                Our Mission
              </p>
              <p className="text-[var(--color-text-muted-70)] font-sans text-[11px] leading-[1.75]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 border-t border-[var(--color-border)] relative z-10">
        {stats.map((stat, i) => (
          <motion.div
            ...
            className={`flex flex-col items-center justify-center py-10 md:py-14 border-[var(--color-border)] hover:bg-[var(--color-surface-hover)] transition-colors duration-500 cursor-default group ${
          ...
          >
            ...
            <p className="text-[var(--color-text-muted-40)] font-sans text-[9px] text-center leading-snug whitespace-pre-line tracking-[0.18em] uppercase group-hover:text-[var(--color-text-muted-60)] transition-colors duration-300">
              {stat.label}
            </p>
          </motion.div>
        ))}
