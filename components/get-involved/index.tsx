    ...
    <section id="get-involved" className="w-full bg-[var(--color-bg-secondary)] relative overflow-hidden select-none border-t border-[var(--color-border)] grid-bg">
            ...
            <motion.a
              ...
              className={`group relative rounded-3xl p-8 border flex flex-col justify-between overflow-hidden transition-all duration-500 cursor-pointer min-h-[360px] ${
                action.featured
                  ...
                  : 'bg-[var(--color-bg-primary)]/50 backdrop-blur-sm border-[var(--color-border)] hover:border-white/15 hover:scale-[1.015]'
              }`}
              style={{
            ...
            >
              ...
              <div className="flex items-center justify-between relative z-10">
                <span
                  className={`font-serif font-black text-2xl select-none transition-colors duration-500 ${
                    action.featured ? 'text-deep-forest/20' : 'text-[var(--color-text-primary)]/10 group-hover:text-lime/35'
                  }`}
                >
                  {action.num}
                </span>
                ...
                <div
                  className={`w-10 h-10 rounded-full border flex items-center justify-center transition-all duration-500 group-hover:scale-105 group-hover:rotate-45 ${
                    action.featured
                      ? 'border-deep-forest/15 text-deep-forest bg-[var(--color-bg-primary)]/5'
                      : 'border-[var(--color-border)] text-[var(--color-text-primary)]/50 bg-[var(--color-surface)] group-hover:border-lime/30 group-hover:text-lime'
                  }`}
                >
              ...
              <div className="relative z-10 mt-16">
                <h3
                  className={`font-sans font-black tracking-tight mb-3 transition-colors duration-300 ${
                    action.featured ? 'text-deep-forest' : 'text-[var(--color-text-primary)] group-hover:text-lime-light'
                  }`}
                ...
                <p
                  className={`font-sans text-xs md:text-sm leading-relaxed transition-colors duration-300 ${
                    action.featured ? 'text-deep-forest/75' : 'text-[var(--color-text-primary)]/40 group-hover:text-[var(--color-text-primary)]/70'
                  }`}
                >
                  {action.desc}
                </p>
              ...
              <div 
                className={`absolute left-8 bottom-0 right-8 h-[2px] transition-opacity duration-500 opacity-0 group-hover:opacity-100 ${
                  action.featured ? 'bg-[var(--color-bg-primary)]/20' : 'bg-gradient-to-r from-transparent via-lime to-transparent'
                }`} 
              />
            </motion.a>
          ))}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-20 lg:px-24 py-16 md:py-24 border-t border-[var(--color-border)] text-center">
        ...
        <motion.p
          ...
          viewport={{ once: true }}
          className="font-serif italic font-normal text-[var(--color-text-primary)]/75 leading-[1.6] mx-auto relative z-10 max-w-3xl"
      ...
      <footer className="relative z-10 w-full border-t border-[var(--color-border)] py-8 md:py-10 bg-[var(--color-bg-primary)]/40 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 md:px-20 lg:px-24 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[var(--color-text-primary)] font-sans text-[11px] font-bold tracking-wide">የምስጋና በጎ አድራጎት ድርጅት</p>
              <p className="text-[var(--color-text-primary)]/35 font-sans text-[8px] tracking-[0.2em] uppercase mt-0.5">Yemsigana Charity</p>
            </div>
          </div>

          {/* Copyrights and links */}
          <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8 text-[var(--color-text-primary)]/35 font-sans text-[10px] tracking-wider font-semibold">
          ...
          <div className="flex items-center gap-3">
            {[Facebook, Twitter, Instagram].map((Icon, i) => (
              <a
                key={i} href="#"
                className="w-9 h-9 rounded-full bg-[var(--color-surface)] border border-[var(--color-border)] flex items-center justify-center text-[var(--color-text-primary)]/45 hover:bg-lime hover:text-deep-forest hover:border-lime hover:scale-110 hover:-translate-y-0.5 transition-all duration-300"
              >
